...
class TextPreprocessor:
    ...
    def tokenize(text: str) -> List[str]:
        ...
        if HAS_KONLPY and _okt is not None:
            try:
                return _okt.morphs(cleaned)
            except Exception as e:
                logger.debug("Morphological analysis failed: %s", e)
        return cleaned.split()  # fallback

    @staticmethod
    def extract_keywords(tokens: List[str]) -> Dict[str, int]:
        """어휘 사전 기반 정치 키워드 카운트"""
    ...
    @staticmethod
    def sentiment(tokens: List[str]) -> Dict[str, float]:
        """긍정/부정 어휘 비율"""
        tokens_set = set(tokens)
...
class BiasClassifier:
    ...
    def __init__(self, model_path: str = "bias_model.pkl") -> None:
        self.model_path = model_path
        self.pipeline: Optional[LogisticRegression] = None
    ...
    def crawl_and_build_dataset(self, max_per_outlet: int = 50) -> List[Dict[str, Any]]:
        """
        ...
        for stance, outlets in MEDIA_DB.items():
            ...
            for cat in NEWS_CATEGORIES:
                if collected >= max_per_outlet:
                    break
                try:
                    query_term = f"{cat} {list(outlets.values())[0]}"
                    resp = requests.get(
                        "https://openapi.naver.com/v1/search/news.json",
                        headers=headers,
                        params={"query": query_term, "display": 20, "sort": "date"},
                        timeout=8,
                    )
                    if resp.status_code != 200:
                        logger.warning("Naver News API returned status %d for %s", resp.status_code, query_term)
                        continue
                    ...
                    logger.error("Request failed during news crawl for %s: %s", cat, e)
                    continue
    ...
    def train(self, dataset: Optional[List[Dict[str, Any]]] = None,
              save_after: bool = True) -> Dict[str, Any]:
        """
        전체 파이프라인 학습
        ...
        if not dataset:
            ...
            return {"accuracy": 0.0, "f1_macro": 0.0, "report": {}, "dataset_size": 0}
    ...
    def predict(self, title: str, description: str = "") -> Dict[str, Any]:
        ...
        """
        if self.pipeline is None or self.tfidf is None or self.lda is None:
            raise RuntimeError("모델 또는 전처리기가 정상적으로 로드되거나 학습되지 않았습니다.")
    ...
    def save(self, path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info("모델 저장 성공: %s", p)
            return True
        except (OSError, pickle.PicklingError) as e:
            logger.error("모델 저장 실패 (%s): %s", p, e)
            return False
    ...
    def load(self, path: Optional[str] = None) -> bool:
        ...
        fpath = Path(p)
        if not fpath.exists():
            logger.warning("모델 파일이 존재하지 않습니다: %s", p)
            return False
        try:
            with open(p, "rb") as f:
                data = pickle.load(f)
            ...
            self.lda = data.get("lda")
            
            # 유효성 검사
            if self.pipeline is None or self.tfidf is None or self.lda is None:
                logger.error("모델 파일이 손상되었거나 누락된 키가 있습니다.")
                return False
                
            logger.info("모델 로드 성공: %s", p)
            return True
        except (OSError, pickle.UnpicklingError, KeyError, Exception) as e:
            logger.error("모델 로드 실패 (%s): %s", p, e)
            return False
